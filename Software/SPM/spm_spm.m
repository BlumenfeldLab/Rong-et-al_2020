...
% nBeta for Beta image outputing in line 838.

% Furthermore, the design matrix picture that the results manager gives is
% the design matrix without the voxel models. The voxel (event) model is
% processed between lines 738 - 761

% The mean regressor is loaded into memory at line 621

% for second level, use the normal spm.
...
%==========================================================================
xX.xKXs   = spm_sp('Set',spm_filter(xX.K,W*xX.X));       % KWX, %get the whitened and filtered design matrix here
...
if ~isfield(xVi,'V')
    ...
    %spm_sp('ox',xX.xKXs) = xX.xKXs.u; U in UV decomposition, changes bases
    xCon   = spm_FcUtil('Set',Fcname,'F','iX0',iX0,xX.xKXs);
    %xCon.X0.ukX0 = spm_sp('ox',xX.xKXs)' * spm_sp('Xi',xX.xKXs,iX0);
    %orthonormal basis' * unused column. (u'*last column) last column in the new rotated basis)
    %xCon.X1o.ukX1o  = spm_sp('cukx',xX.xKXs);  
    %sigma*v' (rotation and scaling to new basis)
    X1o    = spm_FcUtil('X1o', xCon(1),xX.xKXs);%"test space"
    % spm_sp('ox',sX)*Fc.X1o.ukX1o;  => xX.xKXs reconstructed back in the
    % original basis, without the effects of the last column. 
    Hsqr   = spm_FcUtil('Hsqr',xCon(1),xX.xKXs);
    %sf_Hsqr(Fc,sX); hsqr = spm_sp('ox',spm_sp('set',Fc.X1o.ukX1o))' * spm_sp('cukx',sX)
    %basis of xX.xKXs (using SVD)  * sigma*v'
    %normalized error of the pseudoinverse. 
    trRV   = spm_SpUtil('trRV',xX.xKXs);
    %trRV = spm_sp('size',sX,1);/degrees of freedom. 
...
%-Cycle over bunches blocks withi-n planes to avoid memory problems
%==========================================================================
spm_progress_bar('Init',100,str,'');
 

%% VOXEL REGRESSOR GOES HERE

load('/Mean_model.mat') % Path to the location of the mean model
block_regressor = reshape(mean_run_timeslice,[406,47*56*46]); % Reshape the data according to the dimensions of the fMRI volumes
...
for z = 1:nbz:zdim                       %-loop over planes (2D or 3D data)
    ...
    for bch = 1:nbch                     %-loop over blocks
        ...
        for i = 1:nScan
            ...
            Y(i,Cm)  = spm_get_data(VY(i), xyz(:,Cm), false); %this is the image of n scans.
...
%         pause;
        ...
        block_regressor = block_regressor(:,Cm); %event_regressor = event_regressor(:,Cm);%voxel_regressor_slices = voxel_regressor_slices(:,Cm); %added jryu 12/20/2016
        ...
        if CrS
            ...
            fprintf('%s%30s',repmat(sprintf('\b'),1,30),'...estimation over voxels');
            % save here the design matrix, called SPM.xX.X (xX_voxel, if it is not empty)
            voxel_regressor_slices = block_regressor;
            beta = zeros(9,CrS);
            res  = zeros(406,CrS);
            voxel_error = zeros(1,CrS);
            for voxel_num = 1:CrS
                
                % xX_voxel =  xX.X; 
                % xX_voxel(:,1) = block_regressor(:,voxel_num); %replace the first regressor %% Replace this so that it appends the regressor
                % xX_voxel(:,2) = event_regressor(:,voxel_num); %replace the second regressor
                xX_voxel = [voxel_regressor_slices(:,voxel_num), xX.X]; % append the regressor new design matrix
                xKXs_voxel   = spm_sp('Set',spm_filter(xX.K,W*xX_voxel)); %filted and whitened
                pKX_voxel    = spm_sp('x-',xKXs_voxel);                        % projector/pseudoinverse
                beta(:,voxel_num) = pKX_voxel*KWY(:,voxel_num);             %-Parameter estimates
                res(:,voxel_num)   = spm_sp('r',xKXs_voxel,KWY(:,voxel_num));       %-Residuals
                
                if ~isfield(xVi,'V')
                    Fcname = 'effects of interest';
                    iX0_voxel    = [SPM.xX.iB+1 SPM.xX.iG]; %we have one more regressor %% EDITED TO REMOVE THE PLUS 1 VALUE AFTER SPM.xX.iB
                    xCon_voxel   = spm_FcUtil('Set',Fcname,'F','iX0',iX0_voxel,xKXs_voxel);
                    X1o_voxel    = spm_FcUtil('X1o', xCon_voxel(1),xKXs_voxel);
                    Hsqr_voxel   = spm_FcUtil('Hsqr',xCon_voxel(1),xKXs_voxel);
                    %trRV_voxel   = spm_SpUtil('trRV',xKXs_voxel); %constant
                    %throughout voxels
                    %trMV_voxel   = spm_SpUtil('trMV',iX0_voxel); %constant
                    %throughout voxels
                    voxel_error(voxel_num) = sum((Hsqr_voxel*beta(:,voxel_num)).^2,1);
                end
            end
            ...
            if ~isfield(xVi,'V')
                ...
                j   = voxel_error/trMV > UF*ResSS/trRV;
                j   = find(j);
                if ~isempty(j)
                    q  = size(j,2);
                    s  = s + q;
    ...
    if isfield(xX,'W')
        ...
        %-Write beta images
        %------------------------------------------------------------------
        for i = 1:(nBeta)  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Edited here to ensure the extra beta image is written, resulting in the contrast being produced
